"""Populate player and player_team tables from raw JSON data."""
...
def populate_players_from_json():
    """Extract and populate player data from raw JSON."""
    ...
    try:
        ...
        # Get all games with raw JSON data
...
def update_players_table(all_players, cur):
    ...
    for person_id, player_data in all_players.items():
        ...
        if updated_count % 100 == 0:
            print(f"    Updated {updated_count}/{len(all_players)} players...")
    
    print(f"    ✅ Updated {updated_count} players total")
